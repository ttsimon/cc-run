...
多个 profile 匹配到同一个子串时，ccr 会弹出选择器让你挑一个。解决方法：
...
| ccr 如何使用 | **只读**，自动读入 | 合并展示，同名消歧 |

**一句话**：cc-switch 管"这台机器有哪些后端"，ccr 管"这个终端用哪个"。ccr 自动读取 cc-switch 的配置（不会修改），同时允许你通过自定义目录添加额外后端。
